...
Tyto dva grafy představují základní přehled o používání aplikace a akvizici nových uživatelů. Každý provozovatel webové apliakce by tyto grafy měl mít každý den na očích a v okamžiku poklesu okamžitě zjišťovat, co je jeho původem. Na druhou stranu je také zásadní, aby počet aktivních uživatelů stoupal a stejě tak i počet nových uživatelů.

Každý provozovat webové aplikace může zjistit kolik má v daném okamžiku uživatelů. Pokud chce vědět jaké jsou počty aktivních uživatelů a nových uživatelů každý den musí si vést ručně evidenci, nebo na své stránky zavést měřící kód například Google Analytics a nakonfigurovat cíle konverze.

S mým produktem jakékoli konfigurace a nastavování toto odpadá a výsledky jsou "chytře" vyextraktovány z dostupných informací o zákazníkovi. Tím, že se produkt zabývá pouze měřením webových aplikací tedy odpadá veškeré ruční nastavování.
...
\begin{figure}[h]
	\centering
	\includegraphics[width=14cm]{img/ud_chumly_define_features_blurry.pdf}
	\caption{Noví uživatelé Chum.ly}
