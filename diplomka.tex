...
\begin{comment}
TODO
Vytvořit apliakci, která...
Seznámit se s platformou appengine, nasadit na Chum.ly, naučit se Mirah.
\end{comment}
...
\begin{itemize}
	\item Zanalyzujte a popište možnosti monitorování chování návštěvníků webových stránek a aplikací, a to s důrazem na získané informace a metody jejich zpracování.
...
V roce devadesát pět znikl Analog - nástroj pro analýzu logovacích souborů. Jeho autor Stephen Turner ho poskytoval zdarma jako freeware pro několik platform. Jednalo se o první sofistikovaný nástroj pro analýzu a zobrazení návštěvnosti webových stránek. V tabulce \ref{table:analog_output} je znázorněno, jak vypadal jeden z prvních textových výstupů programu analog.
...
\begin{lstlisting}[label=src:Plain,caption=Adresa ve vyhledávači pro dotaz web analytics]
...
Datastore tedy tlačí na programátora, aby zjednodušil logiku aplikace a omezil složitost databázového schéma. Veškeré dotazování a třídění záznamů se pak děje pomocí předdefinovaných indexů. Dokud uživatel nedefinuje index, nemůže podle atributu dotazovat, nebo seřazovat záznamy.
...
Známé funkce sledované aplikace také umožňují identifikovat akce provedené uživatelem. Jednotlivé akce, které uživatel dělá se pak shlukují do jednotlivých návštěv. Jelikož výpisy návštěv obsahují informace o tom, co uživatel zhruba dělal, říkám jim příběhy.
